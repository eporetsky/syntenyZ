  ...
  tabPanel("Synteny Plot",
    sidebarLayout(position="left", fluid=F,
      sidebarPanel(width = 4,
        ...
        bucket_list(
          header = "Select lines for analysis",
...
syntenyZplot <- function(input, output, session, project_name) {
  ...
  update_plot <- reactive({
    ...
    seg_names <- isolate(input$seg_names)
...
MygenoPlotR <- function(
  ...
  for (f in 1:(length(seg_names))){
    ...
    custom_gff <- read.table(paste("projects/PROPEP4/custom_loci/custom_loci_",seg_names[f],".csv",sep=""), sep=",",header=T,row.names=NULL)
...
gene_colors <- read.table("projects/PROPEP4/assigned_features.csv",sep=",",header=T)
...
xlim_func <- function(seg_name, do_gene_xlim=T){
  ...
  xlims_df <- read.table("Projects/pROPEP4/assigned_xlims.csv",header=T,sep=",") # default xlim table
